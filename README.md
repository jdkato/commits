...
rules here pass and fail the messages the tool's tests do, and
`script/differential/run.py` runs the tools themselves beside Vale on one
corpus and diffs the verdicts. `./test.sh` runs
